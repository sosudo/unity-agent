...
- **`lean_run_code` is reserved for:** trying a small, self-contained term/tactic with **minimal imports** (e.g., `import Mathlib.Analysis.Convex.Caratheodory` — not `import Mathlib`). Even then, prefer testing against the real file via `Edit` + `lean_goal`. Subagents must not chain `lean_run_code` calls — one per turn at most.
